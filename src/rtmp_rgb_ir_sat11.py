...
def status_text():
    ...
    head = _latest["head"]
    return jsonify({"text": status_message, "Heding": head})
...
def bearing_deg(lat1, lon1, lat2, lon2):
    ...
    status_message = f"Heading: {headf}"
    return  headf # 0=North, clockwise

# Pixel -> coordinate 1
def calculatecoords(xPixVal, yPixVal):
    global status_message
    # Constants
    dFOV = 54.143 * math.pi / 180 # degrees, 54.143 for 1.7x zoom, 82 for 1x zoom
    xPixMax = 720 
    yPixMax = 640
    height = 40 # Put height of the drone during the flight in meters
    pitch = 90 * math.pi / 180 - 0.000001 # INPUT pitch of the camera, facing down: 90, facing 45: 45, facing forward: 0
    ...
    status_message = f"Heading: {heading}, Pitch: {pitch}, meters north: {mn}, meters east: {me} Estimated coord of detection: {latF}, {lonF}"
    return latF, lonF
...
def main():
    ...
    # YOLO (RGB)
    ap.add_argument("--weights", default="yolo11m.pt", help="YOLO weights (.onnx/.pt/.engine)")
    ...
    ap.add_argument("--classes", type=int, nargs="*", default=0, help="Optional class filter, e.g. 0 1")
    ...
    while True:
        ...
        if has_intersection and _latest["lrf_lat"] is not None and _latest["lrf_lon"] is not None:
            ...
            if first_matching_yolo_box:
                ...
                _log_detection_centroid(centroid_lat, centroid_long)
